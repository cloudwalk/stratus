...
/// Emits instrumentation generated by [`timed`] only when metrics are enabled.
#[doc(hidden)]
#[cfg(feature = "metrics")]
#[macro_export]
macro_rules! __metrics_enabled {
    ($($tokens:tt)*) => {
        $($tokens)*
    };
}

/// Removes instrumentation generated by [`timed`] when metrics are disabled.
#[doc(hidden)]
