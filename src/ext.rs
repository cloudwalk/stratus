    ...
    ($type:ty) => {
        paste::paste! {
            ...
            pub fn [<bincode_ $type:snake>]() {
                ...
                let binary = bincode::serde::encode_to_vec(&value, bincode::config::legacy()).unwrap();
                let (deserialized, _) = bincode::serde::decode_from_slice::<$type, _>(&binary, bincode::config::legacy()).unwrap();
