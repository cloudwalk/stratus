...
fn parse_to_param_types(signature: &str) -> Result<Vec<ParamType>, DecodeInputError> {
    ...
    for param_token in param_tokens {
        param_types.push(parse_solidity_type(&param_token)?);
    }
    Ok(param_types)
}
...
fn parse_solidity_type(type_str: &str) -> Result<ParamType, DecodeInputError> {
    match type_str {
        s if s.ends_with("[]") => {
        ...
        s if s.starts_with('(') && s.ends_with(')') => {
            ...
            for element_token in element_tokens {
                tuple_types.push(parse_solidity_type(&element_token)?);
            }
