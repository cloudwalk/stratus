...
const PARALLEL_BLOCKS: usize = 16;

pub trait DataFetcher: Send + Sync + Sized {
    type FetchedType: Send + 'static;
    type PostProcessType: Send + 'static;

    fn fetch(&self, block_number: BlockNumber) -> impl Future<Output = Self::FetchedType> + Send;

    fn post_process(&self, data: Self::FetchedType) -> impl Future<Output = anyhow::Result<Self::PostProcessType>> + Send;

    /// Generic block fetcher that handles the common logic of fetching blocks in parallel
    fn run(self, backlog_tx: mpsc::Sender<Self::PostProcessType>, mut importer_block_number: BlockNumber) -> impl Future<Output = anyhow::Result<()>> + Send {
        async move {
