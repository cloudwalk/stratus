...
impl PermanentStorage for PostgresPermanentStorage {
    ...
    async fn increment_block_number(&self) -> anyhow::Result<BlockNumber> {
        ...
        let nextval = sqlx::query_file_scalar!("src/eth/storage/postgres/sql/permanent/select_current_block_number.sql")
            .fetch_one(&self.pool)
            .await
            .unwrap_or_else(|err| {
    ...
    async fn read_account(&self, address: &Address, point_in_time: &StoragePointInTime) -> anyhow::Result<Option<Account>> {
        ...
        let account = match point_in_time {
            StoragePointInTime::Present => {
                ...
                sqlx::query_file_as!(Account, "src/eth/storage/postgres/sql/permanent/select_account.sql", address.as_ref())
            ...
            StoragePointInTime::Past(number) => {
                ...
                sqlx::query_file_as!(
                    Account,
                    "src/eth/storage/postgres/sql/permanent/select_account_at_block.sql",
                    address.as_ref(),
                    block_number as _,
                )
                .fetch_optional(conn.for_sqlx())
        ...
        let slot_value_vec: Option<Vec<u8>> = match point_in_time {
            StoragePointInTime::Present =>
                sqlx::query_file_scalar!(
                    "src/eth/storage/postgres/sql/permanent/select_slot.sql",
                    address.as_ref(),
            ...
            StoragePointInTime::Past(number) => {
                let block_number: i64 = (*number).try_into()?;
                sqlx::query_file_scalar!(
                    "src/eth/storage/postgres/sql/permanent/select_slot_at_block.sql",
                    address.as_ref(),
                    slot_index_u8.as_ref(),
                    block_number as _,
                )
    ...
    async fn read_slots(&self, address: &Address, indexes: &SlotIndexes, point_in_time: &StoragePointInTime) -> anyhow::Result<HashMap<SlotIndex, SlotValue>> {
        ...
        let slots = match point_in_time {
            StoragePointInTime::Present =>
                sqlx::query_file_as!(
                    Slot,
                    "src/eth/storage/postgres/sql/permanent/select_slots.sql",
                    indexes_as_vec as _,
                    address as _
                )
                .fetch_all(&self.pool)
            ...
            StoragePointInTime::Past(block_number) =>
                sqlx::query_file_as!(
                    Slot,
                    "src/eth/storage/postgres/sql/permanent/select_historical_slots.sql",
                    indexes_as_vec as _,
                    address as _,
                    block_number as _
                )
        ...
        match block {
            BlockSelection::Latest => {
                ...
                let logs_query = sqlx::query_file_as!(
                    PostgresLog,
                    "src/eth/storage/postgres/sql/permanent/select_logs_by_block_number.sql",
                    block_number as _
                )
                .fetch_all(&self.pool);
            ...
            BlockSelection::Hash(hash) => {
                let header_query = sqlx::query_file_as!(
                    BlockHeader,
                    "src/eth/storage/postgres/sql/permanent/select_block_header_by_hash.sql",
                    hash.as_ref(),
                )
                .fetch_optional(&self.pool);
                ...
                let transactions_query = sqlx::query_file_as!(
                    PostgresTransaction,
                    "src/eth/storage/postgres/sql/permanent/select_transactions_by_block_hash.sql",
                    hash.as_ref()
                )
                .fetch_all(&self.pool);
                ...
                let logs_query = sqlx::query_file_as!(
                    PostgresLog,
                    "src/eth/storage/postgres/sql/permanent/select_logs_by_block_hash.sql",
                    hash.as_ref()
                )
                .fetch_all(&self.pool);
    ...
    async fn read_mined_transaction(&self, hash: &Hash) -> anyhow::Result<Option<TransactionMined>> {
        ...
        let transaction_query = sqlx::query_file_as!(
            PostgresTransaction,
            "src/eth/storage/postgres/sql/permanent/select_transaction_by_hash.sql",
            hash.as_ref()
        )
        .fetch_one(&self.pool)
        .await;
        ...
        let logs = sqlx::query_file_as!(
            PostgresLog,
            "src/eth/storage/postgres/sql/permanent/select_logs_by_transaction_hash.sql",
            hash.as_ref()
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    async fn read_logs(&self, filter: &LogFilter) -> anyhow::Result<Vec<LogMined>> {
        ...
        let query = include_str!("sql/permanent/select_logs.sql");
    ...
    async fn save_block(&self, block: Block) -> anyhow::Result<(), StorageError> {
        ...
        let block_result = sqlx::query_file!(
            "src/eth/storage/postgres/sql/permanent/insert_entire_block.sql",
            ...
            transaction_batch.contract_address as _,    // $37
            log_batch.address as _,                     // $38
            log_batch.data as _,                        // $39
            log_batch.transaction_hash as _,            // $40
            log_batch.transaction_index as _,           // $41
            log_batch.log_index as _,                   // $42
            log_batch.block_number as _,                // $43
            log_batch.block_hash as _,                  // $44
            log_batch.topic0 as _,                      // $45
            log_batch.topic1 as _,                      // $46
            log_batch.topic2 as _,                      // $47
            log_batch.topic3 as _,                      // $48
            account_batch.address as _,                 // $49
            account_batch.bytecode as _,                // $50
            account_batch.new_balance as _,             // $51
            account_batch.new_nonce as _,               // $52
            account_batch.block_number as _,            // $53
            account_batch.original_balance as _,        // $54
            account_batch.original_nonce as _,          // $55
            account_batch.code_hash as _,               // $56
            account_batch.static_slot_indexes as _,     // $57
            account_batch.mapping_slot_indexes as _,    // $58
            slot_batch.index as _,                      // $59
            slot_batch.value as _,                      // $60
            slot_batch.address as _,                    // $61
            slot_batch.block_number as _,               // $62
            slot_batch.original_value as _,             // $63
            historical_nonce_batch.address as _,        // $64
            historical_nonce_batch.nonce as _,          // $65
            historical_nonce_batch.block_number as _,   // $66
            historical_balance_batch.address as _,      // $67
            historical_balance_batch.balance as _,      // $68
            historical_balance_batch.block_number as _, // $69
            historical_slot_batch.index as _,           // $70
            historical_slot_batch.value as _,           // $71
            historical_slot_batch.address as _,         // $72
            historical_slot_batch.block_number as _,    // $73
        )
        .fetch_one(&mut *tx)
        .await
    ...
    async fn read_mined_block_number(&self) -> anyhow::Result<BlockNumber> {
        ...
        let currval: BigDecimal = sqlx::query_file_scalar!("src/eth/storage/postgres/sql/permanent/select_current_block_number.sql")
            .fetch_one(&self.pool)
            .await
    ...
    async fn save_accounts(&self, accounts: Vec<Account>) -> anyhow::Result<()> {
        ...
        for acc in accounts {
            ...
            sqlx::query_file!(
                "src/eth/storage/postgres/sql/permanent/insert_account.sql",
                acc.address.as_ref(),
                nonce,
                balance,
                bytecode,
            ...
            sqlx::query_file!(
                "src/eth/storage/postgres/sql/permanent/insert_historical_balance.sql",
                acc.address.as_ref(),
                balance,
                block_number as _
            )
            ...
            sqlx::query_file!(
                "src/eth/storage/postgres/sql/permanent/insert_historical_nonce.sql",
                acc.address.as_ref(),
                nonce,
                block_number as _
            )
    ...
    async fn reset_at(&self, number: BlockNumber) -> anyhow::Result<()> {
        sqlx::query_file!("src/eth/storage/postgres/sql/permanent/delete_after_block.sql", number as _)
            .execute(&self.pool)
            .await?;
        ...
        sqlx::query_file!("src/eth/storage/postgres/sql/permanent/update_account_reset_balance.sql")
            .execute(&self.pool)
            .await?;

        sqlx::query_file!("src/eth/storage/postgres/sql/permanent/update_account_reset_nonce.sql")
            .execute(&self.pool)
            .await?;

        sqlx::query_file!("src/eth/storage/postgres/sql/permanent/update_account_slots_reset_value.sql")
            .execute(&self.pool)
            .await?;
    ...
    async fn read_slots_sample(&self, start: BlockNumber, end: BlockNumber, max_samples: u64, seed: u64) -> anyhow::Result<Vec<SlotSample>> {
        ...
        let slots_sample_rows = sqlx::query_file_as!(
            SlotSample,
            "src/eth/storage/postgres/sql/permanent/select_random_slot_sample.sql",
            seed,
            start as _,
            end as _,
            max_samples
