...
async fn run(config: RunWithImporterConfig) -> anyhow::Result<()> {
    ...
    let consensus = Consensus::new(Arc::clone(&storage), config.clone().candidate_peers.clone(), Some(config.clone()), config.address, config.grpc_server_address).await; // in development, with no leader configured, the current node ends up being the leader
