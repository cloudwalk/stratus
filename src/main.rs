...
async fn run(config: StratusConfig) -> anyhow::Result<()> {
    ...
    let consensus = Consensus::new(Arc::clone(&storage), config.clone().candidate_peers.clone(), None, config.address, config.grpc_server_address).await; // for now, we force None to initiate with the current node being the leader
