...
impl KafkaConfig {
    pub fn has_kafka_config(&self) -> bool {
        match self.security_protocol {
            ...
            Some(KafkaSecurityProtocol::SaslSsl) => self.bootstrap_servers.is_some() && self.topic.is_some() && self.client_id.is_some() && self.sasl_mechanisms.is_some() && self.sasl_username.is_some() && self.sasl_password.is_some(),
            Some(KafkaSecurityProtocol::Ssl) => self.bootstrap_servers.is_some() && self.topic.is_some() && self.client_id.is_some() && self.ssl_ca_location.is_some() && self.ssl_certificate_location.is_some() && self.ssl_key_location.is_some(),
            None => false,
        }
    }
}
        ...
        match security_protocol {
            ...
            KafkaSecurityProtocol::SaslSsl => {
                producer = ClientConfig::new()
                    .set("bootstrap.servers", config.bootstrap_servers.as_ref().unwrap())
                    .set("client.id", config.client_id.as_ref().unwrap())
                    .set("sasl.mechanisms", config.sasl_mechanisms.as_ref().expect("sasl mechanisms is required").as_str())
            ...
            KafkaSecurityProtocol::Ssl => {
                producer = ClientConfig::new()
                    .set("bootstrap.servers", config.bootstrap_servers.as_ref().unwrap())
                    .set("client.id", config.client_id.as_ref().unwrap())
                    .set("ssl.ca.location", config.ssl_ca_location.as_ref().expect("ssl ca location is required").as_str())
                    .set("ssl.certificate.location", config.ssl_certificate_location.as_ref().expect("ssl certificate location is required").as_str())
                    .set("ssl.key.location", config.ssl_key_location.as_ref().expect("ssl key location is required").as_str())
                    .create()?;
            }
        }
