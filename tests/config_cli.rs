...
/// Asserts that `--validate-config` accepts the configuration: the binary exits successfully with
/// the expected texts in the output, which is returned for further inspection.
fn assert_config_valid(args: &[&str], content: &str, expected: &[&str]) -> String {
    let mut command = Command::new(env!("CARGO_BIN_EXE_stratus"));
    ...
    let output = run(command);
    drop(config);
    assert!(output.status.success(), "expected the binary to succeed, but it failed");
    let stdout = String::from_utf8_lossy(&output.stdout);
    ...
    assert!(
        ...
        "expected plain output when stdout is not a terminal\n  output: {stdout}"
    );
    stdout.to_string()
}

#[test]
fn test_validate_config_valid_leader() {
    let storage = TempDir::new().unwrap();
    let content = format!(
        r#"
            leader = true
    ...
    let stdout = assert_config_valid(
        &[],
        &content,
        &[
    ...
    assert!(
        !stdout.contains("[importer]"),
        "expected no importer section in leader mode\n  output: {stdout}"
    );
}

#[test]
fn test_validate_config_valid_follower() {
    let storage = TempDir::new().unwrap();
    let content = format!(
        r#"
            follower = true
...
#[test]
fn test_validate_config_leader_ignores_importer() {
    let storage = TempDir::new().unwrap();
    let content = format!(
        r#"
            follower = true
    ...
    let stdout = assert_config_valid(
        &["--leader"],
        &content,
        &["warning: ignoring [importer] config in leader mode", "configuration is valid", "leader = true"],
    );
    assert!(
        !stdout.contains("external_rpc"),
        "expected no importer fields in the final config\n  output: {stdout}"
    );
}

#[test]
fn test_validate_config_incomplete_kafka_fails() {
    let storage = TempDir::new().unwrap();
    let content = format!(
        r#"
            follower = true
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("error: incomplete `[kafka]` configuration: add `kafka.topic`"),
        "expected kafka error in output\n  output: {stdout}"
    );
    assert!(stdout.contains("configuration is invalid"), "expected verdict in output\n  output: {stdout}");
}

#[test]
fn test_validate_config_follower_block_mode_warns() {
    let storage = TempDir::new().unwrap();
    let content = format!(
        r#"
            follower = true
    ...
    assert_config_valid(
        ...
        &content,
        &[
            "warning: conflicting `miner.block_mode`: a follower's miner can only start as external",
            "configuration is valid",
        ],
    );
}
...
#[test]
fn test_validate_config_unknown_field_warns() {
    let storage = TempDir::new().unwrap();
    let content = format!(
        r#"
            leader = true
    ...
    assert_config_valid(
        ...
        &content,
        &[
            "warning: unknown field in config file, ignored | field=misspelled_field",
            "configuration is valid",
        ],
    );
}
...
#[test]
fn test_validate_config_no_mode_fails() {
    assert_config_error(
        &["--validate-config"],
        r#"
            [executor]
...
#[test]
fn test_validate_config_malformed_toml_fails() {
    assert_config_error(&["--validate-config"], "leader =", "failed to parse config file");
}
...
fn test_help_exits_successfully() {
    ...
    assert!(
        ...
        "expected --validate-config in the help output\n  output: {stdout}"
    );
}
