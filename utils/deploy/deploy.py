...
def send_request(url: str, method: str, params: list) -> Dict[str, Any]:
    try:
        ...
        while retry_count < max_retries:
            try:
                ...
                if retry_count == max_retries:
                    raise e
                log(
                    message=f"Request failed, retrying ({retry_count}/{max_retries})...",
                    address=url,
        ...
        log(
            message=f"Error: Unable to connect to {url}. Please check the network connection and the node status.",
            address=url,
            response={"error": str(e)},
            error=True,
        )
        raise DeploymentError(
            f"Unable to connect to {url}", error_type="ConnectionError"
        )
...
def log(
    ...
    colored_message = (
        f"{Color.TIMESTAMP}{timestamp}{Color.RESET} - {color}{message}{Color.RESET}"
    )
    if address:
...
def validate_health(address: str, role: NodeRole) -> None:
    ...
    if health.get("result") != True:
        log(
            message=f"Error: {role.name} node health check failed.",
            address=address,
            response=health,
        )
        raise DeploymentError(
            f"{role.name} node health check failed", error_type="HealthCheckError"
        )
    else:
        log(
            message=f"{role.name} node health check passed.",
            address=address,
            response=health,
        )
...
def toggle_transactions(address: str, enable: bool, role: NodeRole) -> None:
    ...
    if response.get("result") != result_check:
        log(
            message=f"Error: Failed to {action.lower()} transactions on {role.name}.",
            address=address,
            response=response,
        )
        raise DeploymentError(
            f"Failed to {action.lower()} transactions on {role.name}",
            error_type="TransactionToggleError",
        )
    else:
        log(
            message=f"Successfully {action.lower()} transactions on {role.name}.",
            address=address,
            response=response,
        )
...
def toggle_miner(address: str, enable: bool) -> None:
    ...
    if response.get("result") != result_check:
        log(
            message=f"Error: Failed to {action.lower()} miner on node.",
            address=address,
            response=response,
        )
        raise DeploymentError(
            f"Failed to {action.lower()} miner on node", error_type="MinerToggleError"
        )
    else:
        log(
            message=f"Successfully {action.lower()} miner on node.",
            address=address,
            response=response,
        )
...
def has_pending_transactions(
    address: str, total_attempts: int, required_checks: int, sleep_interval: float
) -> bool:
    success_count = 0
    for i in range(1, total_attempts + 1):
        pending_tx_count = send_request(
            url=f"http://{address}",
            method="stratus_pendingTransactionsCount",
            params=[],
        )
        log(
            message=f"Checking pending transactions on LEADER (Attempt {i})...",
            address=address,
        )
        if pending_tx_count.get("result") == 0:
            success_count += 1
            log(
                message=f"No pending transactions on the LEADER node (Attempt {i}).",
                address=address,
                response=pending_tx_count,
            )
        else:
            success_count = 0
            log(
                message=f"Error: There are pending transactions on the LEADER node (Attempt {i}).",
                address=address,
                response=pending_tx_count,
            )
    ...
    log(
        message=f"Error: Failed to confirm no pending transactions on the LEADER node after {total_attempts} attempts.",
        address=address,
    )
    return False
...
def blocks_synced(
    ...
    for i in range(1, total_attempts + 1):
        leader_block_number = send_request(
            url=f"http://{leader_address}", method="eth_blockNumber", params=[]
        )
        follower_block_number = send_request(
            url=f"http://{follower_address}", method="eth_blockNumber", params=[]
        )
        log(
            message=f"Checking block numbers (Attempt {i})...",
            address=leader_address,
            response=follower_block_number,
        )
        ...
        if not previous_block_number:
            ...
            log(
                message=f"First check, setting previous block number (Attempt {i}).",
                address=leader_address,
                response=follower_block_number,
            )
        elif (
            leader_block_number_result == follower_block_number_result
            and leader_block_number_result == previous_block_number
        ):
            import_success_count += 1
            log(
                message=f"FOLLOWER is in sync with the LEADER (Attempt {i}).",
            ...
            previous_block_number = leader_block_number_result
            log(
                message=f"Error: FOLLOWER is not in sync with the LEADER (Attempt {i}).",
                address=follower_address,
                response=follower_block_number,
            )
        ...
        time.sleep(sleep_interval)
    log(
        message=f"Error: Failed to confirm block import on FOLLOWER or new blocks are being generated after {total_attempts} attempts.",
        address=follower_address,
    )
    return False
...
def change_role(address: str, method: str, params: list, role: NodeRole) -> None:
    ...
    if change_role.get("result") != True:
        log(
            message=f"Error: Failed to change to {role.name}.",
            address=address,
            response=change_role,
        )
        raise DeploymentError(
            f"Failed to change to {role.name}", error_type="RoleChangeError"
        )
    else:
        log(
            message=f"Successfully changed to {role.name}.",
            address=address,
            response=change_role,
        )
...
def has_leader(leader_address: str, follower_address: str) -> bool:
    ...
    for node in nodes_to_check:
        ...
        log(message="Checking if node is a LEADER...", address=node)
        if state.get("result", {}).get("is_leader"):
            log(
                message="Error: Node is currently a LEADER.",
                address=node,
                response=state,
            )
            raise DeploymentError(
                f"Node {node} is currently a LEADER", error_type="LeaderCheckError"
            )
        else:
            log(message="Node is not a LEADER.", address=node, response=state)
    return False
...
def main() -> None:
    ...
    parser.add_argument(
        "--current-leader", required=True, help="Current Leader address"
    )
    parser.add_argument(
        "--current-follower", required=True, help="Current Follower address"
    )
    parser.add_argument(
        "--auto-approve", action="store_true", help="Auto approve actions"
    )
        ...
        with open(args.log_file, "a") as log_file:
            ...
            try:
                ...
                if not config.auto_approve:
                    confirmation = input(
                        "Do you want to proceed with pausing transactions? (yes/no): "
                    )
                ...
                toggle_transactions(
                    address=config.leader_address, enable=False, role=NodeRole.LEADER
                )
                toggle_transactions(
                    address=config.follower_address,
                    enable=False,
                ...
                if not has_pending_transactions(
                    address=config.leader_address,
                    total_attempts=config.total_check_attempts,
                    required_checks=config.required_consecutive_checks,
                    sleep_interval=config.sleep_interval,
                ):
                    toggle_transactions(
                        address=config.leader_address, enable=True, role=NodeRole.LEADER
                    )
                    toggle_transactions(
                        address=config.follower_address,
                        enable=True,
                ...
                if not config.auto_approve:
                    confirmation = input(
                        "Do you want to proceed with pausing the miner? (yes/no): "
                    )
                    if confirmation.lower() != "yes":
                        print("Script canceled.")
                ...
                if not blocks_synced(
                    leader_address=config.leader_address,
                    follower_address=config.follower_address,
                    total_attempts=config.total_check_attempts,
                    required_checks=config.required_consecutive_checks,
                    sleep_interval=config.sleep_interval,
                ):
                ...
                if not config.auto_approve:
                    confirmation = input(
                        "Do you want to proceed with changing the Leader to Follower? (yes/no): "
                    )
                    if confirmation.lower() != "yes":
                        print("Script canceled.")
                ...
                change_to_follower_params = [
                    f"http://{config.follower_address}/",
                    f"ws://{config.follower_address}/",
                    config.external_rpc_timeout,
                    config.sync_interval,
                    config.external_rpc_max_request_size_bytes,
                ]
                change_role(
                    address=config.leader_address,
                    method="stratus_changeToFollower",
                    params=change_to_follower_params,
                    role=NodeRole.FOLLOWER,
                )
                ...
                if has_leader(config.leader_address, config.follower_address):
                    log(
                        message="Error: One of the nodes is currently a leader. Aborting leader switch."
                    )
                    return

                if not config.auto_approve:
                    confirmation = input(
                        "Do you want to proceed changing the Follower to Leader? (yes/no): "
                    )
                    if confirmation.lower() != "yes":
                        print("Script canceled.")
                        return
                ...
                toggle_transactions(
                    address=config.follower_address, enable=True, role=NodeRole.LEADER
                )
                toggle_transactions(
                    address=config.leader_address, enable=True, role=NodeRole.FOLLOWER
                )
                ...
                raise DeploymentError(
                    f"Process failed: {str(e)}", error_type="DeploymentError"
                )

    except IOError as e:
        log(message=f"Error opening log file: {e}")
        raise DeploymentError(
            "Error initializing Log", error_type="InitializationError"
        )
